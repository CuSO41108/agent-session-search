...
- `npm run test:workflow-transaction`：46 个测试文件、466 项测试通过。
- `npm run test:workflow-performance`：8 个测试文件、182 项 Hub 与运行时回归测试通过。
- `npm run typecheck`、`git diff --check` 与 `npm run release-note:check` 通过；分支相对 `origin/main` 仅保留一份发布说明。
- Windows 上的仓库全量 `npm test` 仍有 44 项失败，集中在 POSIX 路径/权限、符号链接、伪 CLI 启动和平台个性化断言；事务专项全部通过，Hub 的一项并行时序失败在独立运行时通过。全量套件不能据此标记为绿色。
...
| 保存点写入中断 | 同文件验证残留保存点 staging 不会被当成有效快照 |
| 文件提交前、中、后中断 | 同文件模拟首个原子替换完成后的恢复，并验证重复提交不产生冲突 |
| 隔离目录丢失、磁盘不足或容量不可查询 | 同文件验证恢复拒绝、空间不足拒绝和 `statfs` 失败时 fail closed |
| 用户并发修改、回滚冲突 | 同文件验证三方冲突、保留用户修改和回滚暂停 |
| planned 后中断、apply/receipt 间隙 | `workflow-v2-operation-broker.test.ts` 验证 WAL 顺序与 `applying` 恢复检查 |
| inspect 的 applied/not_applied/unknown | 同文件分别验证恢复为 `applied`、`compensated` 或保持 `unknown` |
| 补偿中断、失败和逆序执行 | 同文件验证补偿前检查、首次失败即停止及逆序 operationId |
| API 超时但远端接受 | apply 失败后通过 inspect 证明 applied，且不重复 apply |
| 不可撤回消息 | `workflow-v2-external-adapters.test.ts` 验证消息不会被伪报为已补偿 |
...
| 并行节点失败 | 同文件验证已启动节点全部收敛，且不再发放下一批节点 |
...
| commit plan 跨步骤恢复 | `workflow-v2-commit-coordinator.test.ts` 验证不可变计划顺序、未知步骤 inspect 后续跑和冲突补偿 |
| 提交点与审批策略 | `transaction.test.ts`、`workflow-runtime.test.ts` 和 `WorkflowPage.test.tsx` 验证 checkpoint 合同与 batch/per-operation 选择 |
...
当前实现已按 `direct → controlled → strict_atomic → HTTP/消息适配器 → 节点验收/保存点` 的顺序接通能力。新建 Workflow 默认使用 `strict_atomic`；旧 Workflow 仍按缺失合同兼容为 `direct`，不会被静默升级。

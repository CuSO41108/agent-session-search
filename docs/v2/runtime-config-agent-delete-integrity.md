...
- 删除 Agent 时，只校验了 Workflow 节点的显式引用，Chat、Task、Team、Workflow 默认 Agent 和审核 Agent 均未覆盖；托管 Agent 还被校验逻辑主动豁免。
...
执行配置 (AgentChannel)
  <- ConfiguredAgent.channelId
       ...
       <- WorkflowDraftState.configuredAgentId
...
- Workflow 默认 Agent、审核 Agent 不在检查范围内。
...
2. 删除 Agent 前，必须确认 Chat、Task、Team、Workflow 默认执行、Workflow 审核和 Workflow 显式节点均未引用它。
3. 用户保存入口必须在 `AgentHub` 状态变更前开启删除校验，失败时不得修改内存状态或持久化数据；内部初始化和迁移可以显式跳过用户删除语义。
4. Renderer 可以保留即时提示，但主进程 IPC 校验是最终约束，不能被页面状态绕过。
...
- Agent 被 Chat、Task、Team、Workflow 默认执行、Workflow 审核或 Workflow 节点使用时，删除失败且原 Agent 仍存在。

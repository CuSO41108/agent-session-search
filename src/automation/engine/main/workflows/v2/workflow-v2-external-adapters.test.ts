...
describe("Workflow V2 external operation adapters", () => {
  test("binds HTTP writes to idempotency and requires inspect plus compensation in strict mode", async () => {
    ...
    const prepared = await adapter.prepare({
      ...
      plan: {
        mode: "strict_atomic",
        request: { url: "https://example.test/items", method: "POST", body: "{}" },
    ...
    })).rejects.toThrow("durable credential reference");
  ...
  test("rejects redacted recovery credentials before reaching an HTTP endpoint", async () => {
  ...
  test("invalidates message approval after any bound draft field changes", async () => {
    ...
    await expect(adapter.prepare({ ...prepareContext, plan: { mode: "controlled", draft, approval } })).resolves.toMatchObject({ adapterId: "message" });
    await expect(adapter.prepare({ ...prepareContext, plan: { mode: "controlled", draft: { ...draft, body: "Changed" }, approval } })).rejects.toThrow("changed after approval");
  });

  test("rejects strict messages before sending when the provider cannot retract", async () => {
    const provider = { send: vi.fn(), inspect: vi.fn(async () => "unknown" as const) };
    const adapter = new WorkflowV2MessageOperationAdapter(provider);
    const draft: WorkflowMessageDraft = { channel: "email", recipients: ["user@example.test"], body: "Ready", attachments: [] };
    const approval = { approvalId: "approval-1", actor: "operator", approvedAt: 1, draftDigest: workflowMessageDraftDigest(draft) };

    await expect(adapter.prepare({ ...prepareContext, plan: { mode: "strict_atomic", draft, approval } })).rejects.toThrow("retract support");
  ...
  test("does not claim irreversible messages were compensated", async () => {
    ...
    const prepared = await adapter.prepare({ ...prepareContext, plan: { mode: "controlled", draft, approval: { approvalId: "approval-1", actor: "operator", approvedAt: 1, draftDigest: workflowMessageDraftDigest(draft) } } });

...
export function buildWorkflowAgentPrompt({ workflowId, objective }: WorkflowAgentPromptInput): string {
  ...
  return [
    ...
    "- Model Runtime Review as definition.reviewGates, never as reviewer DAG nodes and never with review back-edges. One execution node may have at most one Review Gate.",
    "- Assess every execution node's criticality. Attach a Review Gate to every critical node that requires runtime quality enforcement. A single Gate must contain all required review dimensions for that node; judgeDimensions is mandatory and must be non-empty.",
    "- Every Review Gate must explicitly set configuredAgentId to an available Configured Agent chosen independently for that node, reviewLevel=low|medium|high, maxQualityRetries from 0 to 5 (default 2), and optional requiredTools containing only tools needed for read-only evidence collection.",
...
export function buildWorkflowRevisionPrompt(input: { workflowId: string; revision: number; definition: unknown; request: string }): string {
  return [
    ...
    "Preserve Review Gates as attached control-plane definitions, not DAG nodes. Any changed critical node must retain or receive one explicit Gate with a configured Agent and non-empty review dimensions; never create nested review.",

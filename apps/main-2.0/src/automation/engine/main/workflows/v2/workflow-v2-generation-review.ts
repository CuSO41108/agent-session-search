...
export function workflowV2GenerationReviewPrompt(input: { definition: WorkflowV2Definition; revision: number; conversation?: WorkflowGrillMessage[] }): string {
  ...
  return [
    ...
    "Challenge missing or redundant nodes, over- or under-decomposition, invalid topology, wrong execution modes, deterministic work assigned to LLMs, incomplete typed inputs or outputs, weak completion criteria, understated script risk, missing Review Gate coverage on critical nodes, and concrete user-experience failure paths.",
    "Review every configured Review Gate as an attached control-plane contract: it must target exactly one real execution node, use an explicit available Agent, include non-empty dimensions, use an appropriate low/medium/high threshold and retry limit, request only read-only evidence tools, and avoid reviewer DAG nodes, back-edges, nested review, or irreversible side effects that cannot be rolled back or compensated.",

  ...
  return <div className="workflow-revision-backdrop" role="presentation" onClick={props.onClose}>
    <section className="workflow-revision-dialog" role="dialog" aria-modal="true" aria-label="Edit workflow definition" onClick={(event) => event.stopPropagation()}>
      ...
      {parsedDefinition?.nodes.some((node) => node.execModel === "llm") ? <div className="workflow-node-agent-config-list">
        {parsedDefinition.nodes.filter((node) => node.execModel === "llm").map((node) => <label key={node.id}><span>{node.title}</span><select aria-label={`Agent for ${node.title}`} value={node.execModel === "llm" ? node.configuredAgentId ?? "" : ""} disabled={parsingPending} onChange={(event) => selectAgent(node.id, event.currentTarget.value)}><option value="">System default</option>{props.configuredAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name} · {agent.modelId}</option>)}</select></label>)}
      </div> : null}

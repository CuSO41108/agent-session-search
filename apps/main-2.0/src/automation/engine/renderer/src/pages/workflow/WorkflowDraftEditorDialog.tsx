        ...
        {parsedDefinition.nodes.filter((node) => node.execModel === "llm" && node.role !== "reviewer").map((node) => {
          ...
          return <div className="workflow-review-gate-config" key={gate.id}>
            ...
            <div className="workflow-review-gate-grid">
              ...
              <label><span>Required read-only tools</span><input value={(gate.requiredTools ?? []).join(", ")} placeholder="tool_a, tool_b" onChange={(event) => replaceDefinition(updateWorkflowReviewGate(parsedDefinition, gate.id, (current) => { const tools = event.currentTarget.value.split(",").map((item) => item.trim()).filter(Boolean); if (tools.length) current.requiredTools = [...new Set(tools)]; else delete current.requiredTools; }))} /></label>
            </div>
